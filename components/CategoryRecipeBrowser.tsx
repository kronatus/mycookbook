...
interface CategoryRecipeBrowserProps {
  category: string;
}

export function CategoryRecipeBrowser({ category }: CategoryRecipeBrowserProps) {
  const [recipes, setRecipes] = useState<Recipe[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [filters, setFilters] = useState<RecipeFiltersState>({
    categories: [category], // Pre-filter by the current category
    tags: [],
  });
  ...
  const handleFiltersChange = (newFilters: RecipeFiltersState) => {
    // Always maintain the category filter
    setFilters({
      ...newFilters,
      categories: [category]
    });
  ...
  const clearFilters = () => {
    setFilters({
      categories: [category], // Keep the category filter
      tags: [],
    });
    setSearchTerm('');
  };
    ...
    <div className="space-y-6">
      ...
      {showFilters && (
        <RecipeFilters
          ...
          availableCategories={[]} // Don't show category filters since we're already in a category
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">
            {hasActiveFilters ? 'No recipes found' : `No ${category} recipes yet`}
          </h2>
          <p className="text-gray-600 mb-6">
            {hasActiveFilters 
              ? 'Try adjusting your search terms or filters to find more recipes.'
              : `Start adding ${category} recipes to see them organized here.`
            }
          </p>
          {hasActiveFilters ? (
            <button
...
// Helper function to extract available tags from recipes
